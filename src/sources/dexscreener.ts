...
export async function fetchDexScreener(query = "solana"): Promise<SourceToken[]> {
  const url = `https://api.dexscreener.com/latest/dex/search?q=${encodeURIComponent(query)}`;

  const res = await safeRequest("dexscreener", async () => await http.get(url));
  if (!res) return []; // circuit open or failed after retries
